...
void CornerDetAC::detectCorners(Mat &Src, vector<Point> &resultCornors, Corners& mcorners,  dtype scoreThreshold, bool isrefine)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		//这里写错了吧，论文公式是 mu - min(fc,fd)，所以应该调用getMax()
...
void CornerDetAC::savecorners(Corners& mcorners, char * filename)
{
	FILE  *fq;
...
void CornerDetAC::readcorners(Corners& mcorners, char * filename)
{
	FILE  *fq;
